...
for k=1:numel(outs), specifyOutput(c,outs{k},'SampleTime',commTs); end
...
set_param(b{1},'PortPaths',[paths ';'],'PortModes',mat2str([ones(1,11) 2*ones(1,8)]),...
    'PortTimes',csv([-ones(1,11) commTs*ones(1,8)]),'PortSigns',csv(-ones(1,19)),...
...
fprintf('STEP7C_FOC_GENERATION_COMPLETE\n');
end
