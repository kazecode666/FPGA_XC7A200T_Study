function result=step7c_run_dynamic(name)
...
if strcmp(name,'stop')
    ...
    f=fopen(fullfile(root,'docs','reports','step7c','dynamic_stop.txt'),'w'); c=onCleanup(@() fclose(f)); %#ok<NASGU>
...
f=fopen(fullfile(root,'docs','reports','step7c',['dynamic_' name '.txt']),'w'); c=onCleanup(@() fclose(f)); %#ok<NASGU>
