# Step 6E acceptance in the original local project. No bitstream.
...
set reports [file join $root docs reports step6e $token]
if {[file exists $reports]} {error {Use a fresh run label}}
file mkdir $reports
set scratch [file join $root .Xil step6e_$token]
...
proc project_sim {profile demo} {
    global root reports
    set_property generic "PI_PROFILE=$profile DEMO=$demo DEADTIME_CYCLES=25" [get_filesets sim_1]
    launch_simulation
    source [file join $root FOC_Gates wave_step6e.tcl]
    run all
    set output [read_text [file join $root FOC_Gates FOC_Gates.sim sim_1 behav xsim simulate.log]]
    write_text [file join $reports e6_profile${profile}_demo${demo}.txt] $output
    set marker "ALL STEP 6E FOC GATE TESTS PASSED profile=$profile deadtime=25 base=80 tail=1 full_cycles=80 shutdown=10"
    if {$demo} {set marker {STEP6E_DEMO_PASS samples=24 tail=1 full_cycles=24 stable_inputs=1 deadtime=25}}
    ...
    save_wave_config [file join $root FOC_Gates FOC_Gates.sim profile${profile}_demo${demo}.wcfg]
    close_sim
}
set status [catch {
    ...
    set leg [file join $root motor_control_ip pwm rtl mc_pwm_deadtime_leg.sv]
    set gate_top [file join $root motor_control_ip integration rtl mc_foc_gate_top.sv]
    foreach d {1 25 50} {
        simulate leg_$d mc_pwm_deadtime_leg_tb [list $leg [file join $root motor_control_ip pwm tb mc_pwm_deadtime_leg_tb.sv]] {ALL STEP 6E DEADTIME LEG TESTS PASSED} [list "deadtime=$d" {duty=5 short=3 cancel=3 async_reset=1}] "" [list DEADTIME_CYCLES=$d]
    }
    foreach profile {0 1} {
        simulate d6_$profile mc_foc_pwm_top_tb [concat $foc_sources [list $pwm_source $adapter $top [file join $root motor_control_ip integration tb mc_foc_pwm_top_tb.sv]]] {ALL STEP 6D FOC PWM TESTS PASSED} [list "profile=$profile base=80 tail=1 full_cycles=80 protocols=6"] $vectors [list PI_PROFILE=$profile DEMO=0]
    }
    set project [file join $root FOC_Gates FOC_Gates.xpr]
    set xdc [file join $root FOC_Gates FOC_Gates.srcs constrs_1 new foc_gates_clock.xdc]
    if {![file exists $project]} {
        create_project FOC_Gates [file dirname $project] -part xc7a200tfbg484-2
        set_property target_language Verilog [current_project]
        foreach f [concat $foc_sources [list $pwm_source $adapter $top $leg $gate_top]] {add_files -norecurse $f}
        ...
        add_files -fileset sim_1 -norecurse [file join $root motor_control_ip integration tb mc_foc_gate_top_tb.sv]
        add_files -fileset sim_1 -norecurse [file join $vectors step6d_pwm_vectors.txt]
        set_property top mc_foc_gate_top [get_filesets sources_1]
        set_property top_auto_set 0 [get_filesets sources_1]
        set_property generic {PI_PROFILE=0 DEADTIME_CYCLES=25} [get_filesets sources_1]
        set_property top mc_foc_gate_top_tb [get_filesets sim_1]
        set_property top_auto_set 0 [get_filesets sim_1]
        set_property generic {PI_PROFILE=0 DEMO=1 DEADTIME_CYCLES=25} [get_filesets sim_1]
    ...
    require {[get_property generic [get_filesets sources_1]] eq "PI_PROFILE=0 DEADTIME_CYCLES=25"} {Synthesis profile must be zero}
    require {[get_property top [get_filesets sources_1]] eq "mc_foc_gate_top"} {Design top mismatch}
    require {[get_property top [get_filesets sim_1]] eq "mc_foc_gate_top_tb"} {Simulation top mismatch}
    set sources [get_files -compile_order sources -used_in synthesis]
    require {[llength $sources]==21} {Expected twenty-one synthesis source files}
    ...
    project_sim 1 0
    project_sim 0 1
    write_text [file join $reports regression_summary.txt] {PASS: leg D=1/25/50, E6 profiles0/1 DEMO0 D=25, default DEMO1, original D6 profiles0/1 DEMO0.}
    ...
    if {[get_property STATUS [get_runs synth_1]] ne {synth_design Complete!}} {
        write_text [file join $reports synthesis_execution.txt] "COMPUTED [clock format [clock seconds]]"
        launch_runs synth_1 -jobs 4
        wait_on_run synth_1
    }
    if {![file exists [file join $reports synthesis_execution.txt]]} {write_text [file join $reports synthesis_execution.txt] "REUSED and rechecked [clock format [clock seconds]]; NEEDS_REFRESH=0 required below"}
    ...
    if {[get_property STATUS [get_runs impl_1]] ne {route_design Complete!}} {
        write_text [file join $reports implementation_execution.txt] "COMPUTED [clock format [clock seconds]]"
        launch_runs impl_1 -to_step route_design -jobs 4
        wait_on_run impl_1
    }
    if {![file exists [file join $reports implementation_execution.txt]]} {write_text [file join $reports implementation_execution.txt] "REUSED and rechecked [clock format [clock seconds]]; NEEDS_REFRESH=0 required below"}
