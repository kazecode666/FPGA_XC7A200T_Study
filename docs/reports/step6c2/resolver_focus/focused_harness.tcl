...
set root {C:/Users/lww/.codex/worktrees/step6c2-pi-dq-limiter/FPGA_XC7A200T}
cd $root
...
puts "DEFAULT_RESOLVER_PASS $bin"
set alternate {C:/Users/lww/.codex/worktrees/step6c2-pi-dq-limiter/FPGA_XC7A200T/.Xil/Alternate Vivado Install}
set bin [resolve_simulator_bin $alternate]
require {[string first {Alternate Vivado Install} $bin] >= 0} {Space-path resolution failed}
foreach tool {xvlog xelab xsim} {
    set output [run_command alternate_${tool}_version [list [file join $bin ${tool}.bat] -version]]
    require {[regexp -line {^Vivado Simulator v2026\.1\s*$} $output]} {Alternate version failed}
}
puts "ALTERNATE_SPACE_RESOLVER_PASS $bin"
set missing [file join $scratch {Absent Install}]
set code [catch {resolve_simulator_bin $missing} detail]
require {$code == 1 && [string first {Simulator tool missing:} $detail] == 0} {Missing tools did not fail}
write_text [file join $reports resolver_result.txt] "PASS
DEFAULT=$::env(XILINX_VIVADO)/bin
ALTERNATE=$bin
Alternate wrappers forward version probes only to the real selected installation; no install copied.
MISSING status=$code message=$detail"
puts RESOLVER_FOCUSED_PASS
exit 0
