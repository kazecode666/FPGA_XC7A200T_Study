...
Status: PASS. The final full build completed at 2026-09-19 17:43:00 with process exit 0 and exact `STEP6C2_BUILD_PASS`. All evidence below is from the final code revision unless explicitly labeled development/RED evidence.

## Scope and provenance

Accepted documentation baseline: `74f6128` (C2 docs PR #13), including accepted C1 PR #12 merge `1207f38`. The isolated worktree branch is `step6c2-pi-dq-limiter`; reviewed RTL/TB handoff is `7bc81ce`. Final tested code revision: `080e50e` (portable-project baseline `9724e77`, build hardening, narrow parser repair and active-install simulator resolution included). The tested code revision and final run are recorded below; a subsequent report-only commit is not represented as the revision executed by Vivado.
...
Final acceptance is recorded under `docs/reports/step6c2/final05/`; code commit `080e50e90afd5edbdb4cefd950a11b316ec6ddc2`, run started 2026-09-19 17:25:58. Vivado/XSim 2026.1 build 6511674 and the actual Vivado environment's CPython 3.13.0 were used. Earlier standalone oracle development used CPython 3.14.6; those versions are not conflated.

Actual measured-machine invocation uses the local installation below (choose a fresh report label). The README also supports `vivado` on PATH; another Windows install location does not require a script edit:
...
The build resolves sibling `xvlog.bat`, `xelab.bat` and `xsim.bat` from `XILINX_VIVADO`, which the active Windows Vivado launcher sets from its own installation. Before any project/simulation work, all three files must exist and each actual `-version` invocation must report 2026.1. `provenance.txt` records the running executable, active installation, resolved simulator directory and actual versions; `tool_*_version.txt` retains each command/output/exit. This run resolved `E:/AMDDesignTools/2026.1/Vivado/bin` from the selected launcher, with three exit-0 version checks.

Focused portability evidence is committed under `docs/reports/step6c2/resolver_focus/`: the same production resolver was invoked against a disposable `Alternate Vivado Install` directory containing spaces, with three small forwarding batch wrappers rather than a copied installation. Actual Tcl `exec` calls through all three paths returned version 2026.1 and exit 0. The harness/result/forwarding contents are retained; this focused check is not represented as a second full build, and its unused full-build state remains IN_PROGRESS. `resolver_missing/` runs the actual production script with an absent installation root and proves early `FAILED`, the missing-tool error stack and outer exit 1, before any project or simulation starts. These focused checks supplement the complete final05 execution; they do not claim a second independently installed Vivado distribution was tested.
...
Task 7's earlier R1/R2 fixes retained diagnostic synthesis summaries and enumerated the five actual waveform-display warnings; their full `final04` evidence remains committed and unchanged alongside `final03`. The single final-review R1 fix then removed the hardcoded simulator-install directory and updated the README. Only the build script and README changed in this last source wave, at `080e50e`; final05 reran every required check after that last script edit and is now authoritative. No RTL, testbench, frozen constraint or protected source changed in either review-fix round.
