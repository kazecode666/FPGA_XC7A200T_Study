...
Date: 2026-09-17. Base `main`: `3fd4058` (includes merged Step 6A PR #10).
...
5. Commit `cfbfa25` completed reset/disable and systematic coverage. The new
   ...
   asserting reset at time zero fixed the TB. Final standalone simulation:
...
& E:\AMDDesignTools\2026.1\Vivado\bin\vivado.bat -mode batch -nojournal `
  -log .Xil/step6b_build_final.log -source scripts/step6b_motor_pwm_build.tcl
```

Final batch execution completed on 2026-09-17, process exit **0**, with
`STEP6B_BUILD_PASS`. Both runs reached 100%: `synth_design Complete!` and
`route_design Complete!`. Run suffix: `20260917_200908_9444`.
...
Routed design also uses 15 CARRY4, 1 BUFG and 133 IOB. All 248 routable nets
...
After the successful build, the XPR root path was normalized to `$PPRDIR`;
RTL, TB, XDC and build parameters were unchanged. A four-file relocated copy
...
not a second routed build. Timestamped run metadata remains in the XPR, while
...
A separate read-only RTL/TB review found no actionable defects in carrier,
...
also found no blocking defect. Reviewers did not independently rerun simulation;
