...
`enable=0` 只阻止新握手，已接受事务仍完成，空闲不积分。C2 独自管理状态，在 N+264 成功提交；下游报错不会回滚积分。无效母线不启动子核，返回错误和全零 duty；时序内部错误要求硬件复位后恢复。`reset_n` 异步断言，由系统负责同步释放，硬件复位中止在途计算并清空状态。`pi_reset`、`uq_zero_en` 都是随样本捕获的命令，后者令 dq 中的 q 电压为零，在非零角度下并不代表 v_beta 为零。
